...
// 숫자맞추기 게임을 메소드로 분리해서 유지보수가 쉽도록 만들어보자
...
public class Ex01GuessingGame {
    ...
    // 사용자가 입력한 숫자와 컴퓨터의 숫자를 비교해서 UP 혹은 DOWN을 출력하는 메소드 ---> private 이 붙는다.
    ...
    // 사용자의 최고점수를 출력하는 프로그램
    private static void printScore(int score) {
        if(score == 0) {
            System.out.println("아직 플레이한 기록이 없습니다.");
        }else {
            System.out.printf("현재 최고기록: %02d회\n", score);
        }
    }

    // 현재 점수와 최고점수를 비교해서 더 작은 숫자를 리턴해주는 setBestScore 메소드
    ...
    // 게임 플레이 기능
    // 단, 매번 끝날때마다 최고 점수를 return 해서, 메뉴 메소드에서 최고 점수를 매번 갱신하게 한다.
