...
public class Student {
    ...
    public int calculateSum() {
        ...
        // 왜냐하면 이 메소드를 "실행"하는 객체의 국영수 점수로 총점을 계산하기 때문이다.
    ...
    public boolean equals(Object o) {
        // 현재 파라미터로 넘어온 o 가 이 클래스(Student) 객체인지 확인해서
        // 만약 Student 클래스의 객체가 맞으면 형변환을 통해 Student 객체로 바꿔준다.
        // 그 후 특정 필드들을 비교하여 그 필드들의 값들이 같으면 return true 해준다.
        
        // 1. o가 Student 클래스 객체인지 확인
        //    이 때에는 instanceof 라는 키워드를 통해서 간단히 확인 가능하다
        //    instanceof는 null 이거나 다른 클래스의 객체인 경우 false가 return 된다.
        if(o instanceof Student) {
            // o가 Student 클래스의 객체이므로 형변환도 가능해졌다.
            Student s = (Student)o ;
            // 이 equals() 메소드를 실행하는 객체와
            // o를 형변환한 s의 특정 필드(지금은 id와 name)을 비교해서 모두 같으면 return true 실행한다.
    ...
    public void printInfo() {
        System.out.printf("※%02d번 %s 학생의 성적※\n", id, name);
        System.out.printf("국어: %03d점 영어: %03d점 수학: %03d점\n", korean, english, math);
        System.out.printf("총점: %03d점 평균: %.2f점\n", calculateSum(), calculateAve());
    }
